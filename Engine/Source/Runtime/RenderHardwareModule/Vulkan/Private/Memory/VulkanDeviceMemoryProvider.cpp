...
VulkanDeviceMemoryProvider::VulkanDeviceMemoryProvider(vk::raii::Device &device, vk::raii::PhysicalDevice &physicalDevice,
    const VulkanAllocationConfiguration &vulkanAllocationConfiguration)
: m_device(device), m_physicalDevice(physicalDevice), m_vulkanAllocationConfiguration(vulkanAllocationConfiguration) {
...
VulkanDeviceMemoryProvider::InitializeAllocatorsCollection(VulkanResourceTilingType resourceTiling) {
    ...
    for (uint32_t i = 0; i < m_memoryProperties.memoryTypeCount; i++) {
        allocatorsCollection.m_allocatorBundles.emplace_back(m_device, i, m_vulkanAllocationConfiguration, m_allocator);
    }
